...
private:
	...
	std::map<unsigned int, unsigned int> collisionShapesIdToIndex_;  //!< Mapping the internal collision shape index with the mesh id
public:
	~CollisionShapes();
	...
	void loadCollisionShapes();  //!< Reads collision shapes of level and individual objects and store them for easy access. .bullet files contain serializations of bullet collision shapes exported from Maya using the Dynamic plugin or through Blender.
