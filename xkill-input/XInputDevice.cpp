...
void XInputDevice::setStandardMappings()
{
	...
	if(buttons_.size() >= 14)
	{
		buttons_[2]->addMapping(InputAction::ACTION_B_NEXT_AMMUNITIONTYPE);
		buttons_[0]->addMapping(InputAction::ACTION_B_PREV_AMMUNITIONTYPE);
		buttons_[1]->addMapping(InputAction::ACTION_B_PREV_FIRINGMODE);
		...
		buttons_[7]->addMapping(InputAction::ACTION_B_SHOW_SCOREBOARD);
