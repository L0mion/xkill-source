...
void ManagementInstance::update()
{
...
void ManagementInstance::addRenderAtInstance(Attribute_Render* renderAt)
{
	...
	if(getInstancesFromMeshID(renderAt->meshID, instancedData))
	...
	{ //no existing instanced data of mesh id, create new one.
		instancedData = InstancedData(renderAt->meshID);
		instancedData.addInstance(newInstance);
...
bool ManagementInstance::getInstancesFromMeshID(unsigned int meshID, InstancedData& inout_instancedData)
{
	std::map<unsigned int, InstancedData>::iterator it = instancesMap_.find(meshID);
	if(it != instancesMap_.end())
		inout_instancedData = (*it).second;

	return it != instancesMap_.end();
}
