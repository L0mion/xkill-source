...
HRESULT ManagementFX::initILPosNormTexTanSkinned(ID3D11Device* device)
{
	...
	/*
	const unsigned int nrOfElementsInIed = 6;
	D3D11_INPUT_ELEMENT_DESC ied[nrOfElementsInIed] = 
	{
	...
	hr = device->CreateInputLayout(ied, nrOfElementsInIed, animationVS_->getBlob()->GetBufferPointer(), animationVS_->getBlob()->GetBufferSize(), &ilPosNormTexTanSkinned_);
	*/
